...
function bumpClientWatermark(
  ...
  // Entries (not keys) throughout: re-insertion at the tail makes key order
  // approximate recency, and slicing entries keeps the values attached so no
  // index lookup — and no non-null assertion — is needed.
  const entries: Array<
    [
      string,
      number,
    ]
  > = Object.entries(watermarks ?? {}).filter(([id]) => id !== clientId);
  entries.push([
    clientId,
    seq,
  ]);
...
function withinPayloadCap(payload: unknown): boolean {
  ...
  try {
    ...
    return json === undefined || json.length <= MAX_SET_PAYLOAD_BYTES;
  } catch {
    return false;
  }
}
...
export function openUiSurface(config: OpenUiSurfaceConfig): OpenUiSurfaceLayer {
  ...
  const hooks: ContextLayerHooks<OpenUiSurfaceState> = {
    ...
    async onReturn({ childState, parentState, childCtx }) {
      ...
      return {
        parentState: observeFor(childCtx.threadId)(merged),
      };
    },
