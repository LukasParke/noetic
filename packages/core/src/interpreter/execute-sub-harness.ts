...
interface SubHarnessSessionStore {
   ...
   * the same key (e.g. parallel fork legs) dedupe onto one `doStart` instead
   * of both starting and one leaking. Driving concurrent TURNS on one reused
...
/**
 * Start one session. Kept as a named async function so callers can hold the
 * pending promise — calling it returns synchronously, before the instruction
 * resolve inside it awaits, which is what lets the reuse store be populated
 * without a dedupe window.
 */
...
export async function executeSubHarness<TContext, I, O>(
  ...
  // Idle watchdog (S2): an external coding agent is a real process that can
  // hang (CLI waiting on stdin, SDK deadlock). The model-call path has a
  ...
  // S1: a turn the AGENT reports as failed must not flow downstream as a
  // normal answer. The items/usage are already applied (the spend is real and
  // the transcript is the evidence); the step itself fails so loop onError /
