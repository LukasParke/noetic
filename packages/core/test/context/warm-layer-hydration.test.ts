/**
 * Warm layer hydration: successive turns on one thread carry non-execution-scoped
 * layer state forward IN MEMORY instead of re-reading it from storage.
 *
...
describe('warm layer hydration across turns on one thread', () => {
  ...
  it('separate threads hydrate independently', async () => {
    ...
    // One cold init per thread — the warm key is thread-scoped, so thread B
  ...
  it('a preview between turns does not poison the thread warm pointer', async () => {
     ...
     * the thread's warm source, the next real turn would find a pointer to wiped
    ...
    // ...and, crucially, they did not become the thread's warm SOURCE. Turn 2
