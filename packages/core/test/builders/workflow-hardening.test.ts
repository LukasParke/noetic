 ...
 *
 * W1: `tool` nodes must pass the same gates as the model tool-loop — Zod
 ...
 * W3: dynamic-fork template hydration is cached per index — repeated fork
 *     invocations must not re-hydrate (and re-register) per item per call.
 * W5: branch routes support `matchMode: 'exact'`; substring stays default.
...
describe('W1 — tool nodes pass the model tool-loop gates', () => {
...
describe('W3 — dynamic fork caches per-index hydration across invocations', () => {
...
describe('W5 — branch matchMode', () => {
  function branchNode(matchMode?: 'substring' | 'exact'): WorkflowNode {
    return {
      kind: 'branch',
      id: 'router',
      routes: [
  ...
  function routeOf(matchMode?: 'substring' | 'exact'): (input: string) => {
    ...
    const hydrated = hydrateNode(branchNode(matchMode), makeCtx(new Map()));
    assert(hydrated.kind === 'branch');
...
describe('W6 — dynamicWorkflow feeds hydration errors to the revision loop', () => {
  test('a planner emitting an unknown tool ref gets error feedback and can repair', async () => {
    const { dynamicWorkflow } = await import('../../src/patterns/dynamic-workflow');
