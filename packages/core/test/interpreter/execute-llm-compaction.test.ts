...
describe('executeLLM — the fold index survives the system/tail partition', () => {
  /**
  ...
  it('agrees with the bare no-layer path on the same log', async () => {
    ...
    seed(layered);
    await executeLLM(STEP, '', layered.ctx, LAYERS);

    const bare = makeRig();
    seed(bare);
    await executeLLM(STEP, '', bare.ctx);
...
describe('executeLLM — context_pressure event', () => {
  ...
  it('still fires when the threshold is crossed on a steering retry', async () => {
    ...
    await executeLLM(tightStep, '', ctx, LAYERS);
