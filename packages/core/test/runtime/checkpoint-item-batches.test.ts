 ...
 *
 * Two invariants, both of which a session-length run would otherwise violate
 * silently:
 *
 *  1. Write volume is O(delta), not O(transcript) per turn. Because the session
 *     owns one log across every turn in a thread, the batch owner key must be
 *     the THREAD — keying it by executionId (fresh every turn) resets the delta
 *     watermark each turn and re-writes the whole transcript under a new key.
 *     That is the same O(n²) session cost, just fanned across keys, and it is
 *     invisible to a latency benchmark. The offset assertions below fail on the
 *     execution-keyed shape and pass on the thread-keyed one.
 *  2. Restore stitches the batches back. Capture and restore must derive the
 *     SAME owner key, or restore reads an empty prefix and the resumed session
 *     comes back with no transcript at all.
 */
...
/** A storage adapter that records every `set` so write volume is observable. */
...
/** Offsets, in write order, of every item-log batch key under one owner. */
...
describe('item-log checkpoint batches are O(delta) and thread-keyed', () => {
  it('each turn appends at a strictly increasing offset, never re-writing from 0', async () => {
    ...
    // Offset 0 exactly once — the first turn. Under execution keying every turn
    // starts a fresh watermark and writes offset 0 again, four times over.
    expect(offsets.filter((o) => o === 0)).toHaveLength(1);
    for (let i = 1; i < offsets.length; i++) {
      expect(offsets[i]).toBeGreaterThan(offsets[i - 1] ?? -1);
    }
    // The watermark agrees with the live log.
  ...
  it('later turns write less than the whole transcript (bytes stay per-delta)', async () => {
    ...
    // Every turn appends the same two items, so per-turn bytes are flat rather
    // than growing with transcript length. Compare the last turn to the first
    // with generous slack: the invariant is "does not grow with n", and the
    // O(n) shape would make turn 6 roughly 6x turn 1.
  ...
  it('restore stitches the batches back through the thread owner key', async () => {
    ...
    // A fresh harness over the same store stands in for the restarted process:
    // its watermark map is empty, so the transcript can only come from batches.
    ...
    // ...and the resumed harness's watermark is seeded, so its next checkpoint
    // appends rather than re-writing history.
  ...
  it('a legacy store without appendItems keeps the inline snapshot shape', async () => {
    ...
    // A third-party CheckpointStore predating batches: only the four required
    // methods. `captureCheckpoint` must fall back to inlining the transcript.
    const legacy = {
      save: full.save,
      load: full.load,
      list: full.list,
    ...
    // Nothing was written under a batch prefix.
