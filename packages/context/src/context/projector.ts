 ...
 *
 * The runtime surfaces `overThreshold` as a `context_pressure` framework event.
 * That is the complementary half of the band assembler's budget enforcement:
...
export function historyPressure(
  ...
  const compactAt =
    policy.compactAt ?? Math.floor((policy.tokenBudget - policy.responseReserve) * 0.8);
  return {
    historyTokens,
    compactAt,
