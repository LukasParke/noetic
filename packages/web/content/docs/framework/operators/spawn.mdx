---
title: spawn
description: Run a child step in an isolated context with optional spawn-local context layers.
---
...
You can attach spawn-local context layers via the optional `context` field. These layers use `onSpawn` hooks to provide items to the child and `onReturn` hooks to transform results back to the parent. Spawn-local context fully replaces parent layer propagation, ensuring complete isolation.
...
The `context` field is typed `ContextInput`: it accepts either a raw `ContextLayer[]` array or the config object produced by the `context()` builder. The builder provides typed access to layer state and is the recommended approach.
...
```ts
import { createMessage, spawn, step } from '@noetic-tools/core';
...
const contextLayer: ContextLayer<ResearchState> = {
  id: 'context-layer',
  ...
  hooks: {
    onSpawn: async ({ parentState }) => ({
      ...
      items: [createMessage('Here is background context for your research.', 'user')],
    }),
    onReturn: async ({ parentState, result }) => ({
      parentState: { ...parentState, lastResearch: String(result) },
      result,
    }),
  },
};

const withContextLayers = spawn({
  id: 'research-agent',
  child: step.llm({
    id: 'researcher',
    model: 'openai/gpt-4o',
...
## Real-World Example: Research Delegation with Context Layers

```ts validate=none
import { createMessage, loop, spawn, step, until } from '@noetic-tools/core';
import type { ContextLayer } from '@noetic-tools/core';
...
const briefingLayer: ContextLayer<BriefingState> = {
  ...
  hooks: {
    onSpawn: async ({ parentState }) => ({
      childState: null,
      items: [createMessage(`Research the following topic: ${parentState.topic}`, 'user')],
    }),
    onReturn: async ({ parentState, result }) => ({
...
### Isolating background work from the parent's session
...
Combine detached spawn with the [loop inbox channel](/docs/framework/operators/loop-and-until#inbox-channel) for a full async delegation workflow:
...
handle.await().then((result) => {
  void harness.send(inbox, `[Sub-agent done] ${result}`, parentCtx);
});
...
```ts
import { channel, spawn, step, tool, until } from '@noetic-tools/core';
...
// A tool's execute receives a ToolExecutionContext as its second argument;
// reach the harness and the current Context through it (toolCtx.harness / toolCtx.ctx).
const delegateTool = tool({
  name: 'delegate',
  description: 'Run a sub-agent and wait for its result. Use when you need the answer now.',
  input: z.object({ task: z.string() }),
  output: z.string(),
  execute: async (args: { task: string }, toolCtx) => {
    const child = step.llm<ContextData, string, string>({
      id: 'sync-sub', model: 'openai/gpt-4o', instructions: 'Answer concisely.',
    });
    return toolCtx.harness.run(spawn({ id: 'sync-spawn', child }), args.task, toolCtx.ctx);
  },
});
...
const launchTool = tool({
  ...
  execute: async (args: { task: string }, toolCtx) => {
    const child = step.llm<ContextData, string, string>({
      id: 'async-sub', model: 'openai/gpt-4o', instructions: 'Answer concisely.',
    });
    const handle = toolCtx.harness.detachedSpawn(child, args.task, toolCtx.ctx);
    // Notify the parent loop when the sub-agent finishes
    handle.await().then(
      (result) =>
        void toolCtx.harness.send(inbox, `[Agent ${handle.id} done] ${result}`, toolCtx.ctx),
      (err) =>
        void toolCtx.harness.send(inbox, `[Agent ${handle.id} failed] ${err}`, toolCtx.ctx),
    );
...
const isolated = spawn({
  id: 'sub-agent',
  child: step.llm({ id: 'researcher', model: 'openai/gpt-4o' }),
...
2. **Per-step override** — the `subprocess` field on `StepRun` or `StepSpawn`.
3. **Harness default** — `harness.subprocess` (defaults to `createInMemorySubprocessAdapter()`).

Other step kinds (`llm`, `tool`, `branch`, `fork`, `provide`, `loop`) always use the harness default.

### Step Registry

When an adapter crosses a process boundary, the child runtime must locate the step body by id. Every step builder auto-registers its result in a shared registry (`@noetic-tools/core/runtime/step-registry`). `lookupStep(stepId)` is the cross-process contract — the child process imports the same entry module as the parent and resolves the step by id before executing it.

### Durable Handle Manifests

Adapters configured with a durable `StorageAdapter` persist a manifest per handle: `handleId`, `stepId`, `serializedInput`, `executionId`, and transport-specific identity (pid + `pidStarttime` for OS children; `socketPath` for IPC). On parent restart, `adapter.listLive()` rediscovers the still-running children and `adapter.reattach(handleId)` rebinds each handle. See the [Durability](/docs/framework/durability) page for the full restart flow.

## provide vs spawn

Use [`provide`](/docs/framework/operators/provide) when you want to attach context layers to a subtree without isolating the child context. The child shares the parent's `ItemLog` and conversation history -- layers simply become available to all descendant steps.
...
| Context layers | Inherited by descendants | Spawn-local only |
| Use case | Attach layers to a subtree | Sub-agents, delegation, sandboxing |

## Related

- [provide](/docs/framework/operators/provide) -- attach context layers without context isolation.
- [fork](/docs/framework/operators/fork) -- parallel execution without context isolation.
- [Loop & Until](/docs/framework/operators/loop-and-until) -- use a loop as a spawn child for iterative sub-agents.
- [Overview](/docs/framework/overview) -- how spawn fits into the eight primitives.
